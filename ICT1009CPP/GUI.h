...
{
	const string API = "b492e30c63eb43059d18619a6a2d8fad";
	const string website = "channelnewsasia.com";
	string category[] = { "Singapore" , "Asia", "World","Business", "Sport", "Commentary" };
	string web = "http://newsapi.org/v2/everything?domains=channelnewsasia.com&pageSize=60&apiKey=b492e30c63eb43059d18619a6a2d8fad";

	Document doc;
	CNA cna;
	wstring scrape;
	vector<CNA> cnaarticle;
	//string news = "http://newsapi.org/v2/everything?domains=";
	//news.append(website);
	//news.append("&apiKey=" + API);
	auto r = Get(Url{ web });

	SetConsoleOutputCP(CP_UTF8);

	doc.Parse(r.text.c_str());
	for (SizeType i = 0; i < doc["articles"].Size(); i++) {
		string title;
		string date;
		string content;
		string category;
		...
		cnaarticle.push_back(cna);
		cna.createCSV(cnaarticle);
	}
}
